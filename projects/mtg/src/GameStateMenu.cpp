...
void GameStateMenu::Start()
{
    ...
    // Save the old handle so we can release its lock after re-locking below.
    // bgTexture must remain locked across ClearUnlocked() so the texture is not
    // evicted and forced to reload from disk (which fails after a game session).
    ...
    // Release the previous lock now that the new RETRIEVE_LOCK is held.
    if (oldBg)
