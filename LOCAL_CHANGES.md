...
| 2026-05-10 | feature/duskmourn-rooms    | TBD        | WEvent.{h,cpp} (WEventRoomFullyUnlocked); AllAbilities.{h,cpp} (TrCardRoomFullyUnlocked, AAAlterDoorUnlocked); MTGAbility.cpp (`@roomfullyunlocked` trigger + `unlockdoor:N` parsers); borderline.txt (Bottomless Pool // Locker Room sample); patch_core_zip.py (include borderline.txt + _macros.txt); download_dsk_room_sample.py | Windows build pass; gameplay verification pending | Day-1 Duskmourn Rooms: engine support + one sample DSK Room. DSK 673448 image already in G:\Wagic-windows\User\sets\DSK\DSK.zip |
| 2026-06-27 | wagic-v146-windows         | TBD        | SDLActivity.java (databaseurl → upstream, catch Throwable in download thread); assemble_apk.py (bundle bin/dexedLibs/*.jar as classes2+.dex) | needs RP5 rebuild + retest | Fixes image-downloader crash: jsoup was never packaged (assemble_apk.py only shipped app classes.dex), so the slow scrape path threw NoClassDefFoundError — an Error that bypassed `catch(Exception)` and killed the app. Now jsoup/json-simple/zip4j ship as secondary dex (ART auto-loads), and `catch(Throwable)` degrades any remaining lib/OOM failure to a per-set error. databaseurl repointed at WagicProject (its `/releases/latest/` hosts CardImageLinks.csv; the fork's only release is a prerelease and 404s). |

## Follow-ups / known limitations

- **Image downloader database is upstream-only (2026-06-27).** `databaseurl` now points at `WagicProject/wagic/releases/latest/download/CardImageLinks.csv`. That CSV indexes only upstream sets, so the **fast (indexed) download** covers upstream cards; any **fork-added set** (ECL, TLA, SOS, and other post-upstream content) is **not in the index** and falls back to the slow per-card Scryfall scrape — slower, and it can't fetch cards Scryfall won't map by multiverseid. To fully fix: publish our own `CardImageLinks.csv` (covering fork sets) as an asset on a **non-prerelease** fork release, then repoint `databaseurl` back at `Flounder1664/wagic`. CSV-generation tooling lives in the `wagic-tools` repo.
